...
export const VideoCall: React.FC = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // 2. Upload directly from browser to R2 — never touches your droplet
      ...
      // 3. Tell your backend the upload is complete, get download URL
      const { data: completeData } = await api.post('/upload/complete', { key: data.key });

      setUploadSuccess(true);
      console.log('[Upload] Download URL:', completeData.downloadUrl);
    ...
    <div className="min-h-screen bg-surface">
      ...
      <header className="fixed top-0 w-full z-40 bg-white/80 backdrop-blur-xl border-b border-midnight/5 shadow-sm">
        <div className="flex justify-between items-center w-full px-6 py-4 max-w-7xl mx-auto">
          ...
          {/* Click-to-copy invite link — always visible in the room */}
          <button
            onClick={copyInviteLink}
            title="Click to copy invite link"
            className={`flex items-center gap-2 text-xs font-bold px-3.5 py-2 rounded-xl border transition-all select-none ${
              linkCopied
          ...
          >
            {linkCopied ? (
              <><Check className="w-3.5 h-3.5 flex-shrink-0" /><span>Copied!</span></>
            ) : (
              <>
                <Link2 className="w-3.5 h-3.5 flex-shrink-0" />
                <span className="font-mono max-w-[180px] sm:max-w-[260px] truncate">{inviteLink}</span>
              </>
            )}
          </button>
        </div>
